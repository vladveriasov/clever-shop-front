...
//         try {
//             const result = await axios.get(`https://musefrill-ailon4--111867942298563.stormkit.dev/delivery/${props.deliveryId}`, updatedDelivery);
...
const EditDeliveryForm = (props) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await axios.put(`https://musefrill-ailon4--111867942298563.stormkit.dev/delivery/${props.deliveryId}`, updatedDelivery);
            // Handle successful update (e.g., close the modal, refresh the data)
        } catch (exception) {
            console.log("exception:", exception);
        }
